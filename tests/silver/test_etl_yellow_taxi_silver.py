"""Testes unitários para o etl_yellow_taxi_silver.

Cobre transform e load do pipeline Silver do Yellow Taxi.
...
import src.silver.etl_yellow_taxi_silver as mod
...
@pytest.fixture
def df_bronze_yellow(spark):
    """Retorna um DataFrame Spark simulando a tabela table_yellow_taxi_bronze.
    ...
    Returns:
        DataFrame Spark com schema Bronze do Yellow Taxi.
    """
    data = [
        (1, 2,    15.0, "2023-01-01 08:00:00", "2023-01-01 08:30:00", 2023, 1),
        (2, 1,    25.0, "2023-02-15 09:00:00", "2023-02-15 09:45:00", 2023, 2),
        (1, None, 10.0, "2023-03-10 10:00:00", "2023-03-10 10:20:00", 2023, 3),
        (2, 3,    None, "2023-04-05 11:00:00", "2023-04-05 11:30:00", 2023, 4),
    ]
...
class TestTransform:
    ...
    def test_silver_schema_columns_present(self, df_bronze_yellow):
        """Valida que todas as colunas do schema Silver estão presentes no resultado."""
        result = mod.transform(df_bronze_yellow)
        colunas_esperadas = {
    ...
    def test_vendorid_renamed_to_vendor_id(self, df_bronze_yellow):
        """Valida que vendorid é renomeado para vendor_id."""
        result = mod.transform(df_bronze_yellow)
        assert "vendor_id" in result.columns
        assert "vendorid" not in result.columns

    def test_pickup_renamed_to_pickup_datetime(self, df_bronze_yellow):
        """Valida que tpep_pickup_datetime é renomeado para pickup_datetime."""
        result = mod.transform(df_bronze_yellow)
        assert "pickup_datetime" in result.columns
        assert "tpep_pickup_datetime" not in result.columns

    def test_dropoff_renamed_to_dropoff_datetime(self, df_bronze_yellow):
        """Valida que tpep_dropoff_datetime é renomeado para dropoff_datetime."""
        result = mod.transform(df_bronze_yellow)
        assert "dropoff_datetime" in result.columns
        assert "tpep_dropoff_datetime" not in result.columns

    def test_taxi_type_literal_yellow(self, df_bronze_yellow):
        """Valida que taxi_type é adicionado com valor literal 'yellow'."""
        result = mod.transform(df_bronze_yellow)
        valores = [r["taxi_type"] for r in result.select("taxi_type").collect()]
        assert all(v == "yellow" for v in valores)

    def test_nulls_removed_from_mandatory_columns(self, df_bronze_yellow):
