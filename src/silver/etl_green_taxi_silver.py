"""Script responsável pela transformação e persistência dos dados do Green Taxi na camada Silver do Data Lake.

Este script lê os dados brutos da tabela table_green_taxi_bronze no Glue Data Catalog,
...
logger = logging.getLogger("etl_green_taxi_silver")

# ─── Configurações ──────────────────────────────────────────────────────────
BUCKET_NAME = None
DATABASE_BRONZE = "ifood_case_bronze"
TABLE_BRONZE = "table_green_taxi_bronze"
TABLE_SILVER = "table_green_taxi_silver"
...
def extract(glue_context):
    """Lê a tabela table_green_taxi_bronze do Glue Data Catalog e retorna um DataFrame Spark.

    Args:
        glue_context: Contexto do Glue inicializado no run().

    Returns:
        DataFrame Spark com os dados brutos do Green Taxi.
    """
    logger.info(
...
# ─── Transform ──────────────────────────────────────────────────────────────
def transform(df):
    """Seleciona, renomeia, tipifica e remove nulos do DataFrame do Green Taxi.
    ...
    Aplica as seguintes transformações:
        ...
        - Adição da coluna taxi_type com valor literal 'green'.
